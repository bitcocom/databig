...
CREATE TABLE IF NOT EXISTS `temp`.`t_customer` (
  ...
  INDEX `fk_t_customer_ t_region_idx` (`region_code` ASC) VISIBLE,
  CONSTRAINT `fk_t_customer_ t_region`
...
CREATE TABLE IF NOT EXISTS `temp`.` t_sales` (
  ...
  `product_code` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_ t_sales_t_customer1_idx` (`customer_id` ASC) VISIBLE,
  INDEX `fk_ t_sales_t_product1_idx` (`product_code` ASC) VISIBLE,
  CONSTRAINT `fk_ t_sales_t_customer1`
    FOREIGN KEY (`customer_id`)
    REFERENCES `temp`.`t_customer` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_ t_sales_t_product1`
